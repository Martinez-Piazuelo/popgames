...
class PayoffMechanism:
    ...
    def __init__(
        ...
        check_type(arg=n, expected_type=int, arg_name="n")
        check_scalar_value_bounds(arg=n, arg_name="n", strictly_positive=True)
        self.n = n

        check_type(arg=d, expected_type=int, arg_name="d")
        check_scalar_value_bounds(arg=d, arg_name="d", min_value=0)
        self.d = d
    ...
    def _h_map_wrapped(self, y: np.ndarray) -> np.ndarray:
        """
        ...
        return self.h_map(q, x).reshape(
            self.n,
        )

    def _w_map_wrapped(self, t: float, y: np.ndarray) -> np.ndarray:
        """
        ...
        q = y[: self.d].reshape(self.d, 1)
        x = y[self.d :].reshape(self.n, 1)
        return np.vstack([self.w_map(q, x), np.zeros_like(x)]).reshape(
            self.d + self.n,
        )
