...
def build_auxiliary_matrices(
    ...
    """
    aux_vector = np.array(population_game.masses).reshape(
        population_game.num_populations, 1
    )
...
def compute_vertices(population_game: PopulationGame) -> Optional[np.ndarray]:
    """
    ...
    # Case 2:
        ...
        for c in [
            Z[:, 0],
            -Z[:, 0],
        ]:  # Optimize along both dimensions of the line, i.e., do max and min
            res = sp.optimize.linprog(
                c, A_ub=A_ineq, b_ub=b_ineq, A_eq=A_eq, b_eq=b_eq, method="highs"
            )
            if not res.success:
    ...
    for vertex in vertices_red:
        v = np.clip(
            Z @ vertex.reshape(-1, 1) + x0, 0, np.dot(aux_matrix.T, aux_vector)
        ).reshape(-1)
        vertices.append(v)
    return vertices


def map2delta(population_game: PopulationGame, x: np.ndarray) -> np.ndarray:
    """
