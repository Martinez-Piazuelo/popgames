...
def check_list_length(arg, expected_length, arg_name):
    if not len(arg) == expected_length:
        raise ValueError(
            f"Expected '{arg_name}' to be a list of length {expected_length}, but got length {len(arg)} instead."
        )
...
def check_array_in_simplex(arg, n, m, arg_name, tolerance=1e-6):
    ...
    if sum > m + tolerance or sum < m - tolerance:
        raise ValueError(
            f"Expected sum({arg_name})={m}, but got sum({arg_name})={sum} instead (tolerance={tolerance})."
        )
...
def check_function_signature(
    arg: object,
    expected_input_shapes: list[tuple],
    ...
    try:
        ...
        raise ValueError(
            f"Provided {name} raised an error when called with an input of {len(expected_input_shapes)} np.ndarrays of shapes={expected_input_shapes}"
        )
    ...
    try:
        ...
        raise ValueError(
            f"Provided {name} must return as output a np.ndarray of shape={expected_output_shape} when called with an input of {len(expected_input_shapes)} np.ndarrays of shapes={expected_input_shapes}"
        )
